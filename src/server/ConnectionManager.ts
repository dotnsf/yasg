...
import { ConnectionState, Logger } from '../shared/types';
...
export class ConnectionManager {
  ...
  constructor(maxConnections: number, logger: Logger) {
   ...
   */
  updateState(connectionId: string, state: ConnectionState): void {
    const info = this.connections.get(connectionId);
    if (info) {
      info.state = state;
